...
module.exports = function(games, users) {
        ...
        if(!userInQueue(message.author.id)) {
            ...
            if(!userActiveGame) {
                ...
                if(queueIds.length < 6) {
                    ...
                    message.channel.send('Added ' + users[message.author.id].name + ' to queue. Queue currently has ' + queueIds.length + ' players');
                    if(queueIds.length == 6) {
                        message.channel.send('Queue is now full, creating a match and clearing the queue');
                        ...
                        message.channel.send(
                            'New match created with ID `' + uuid + '` created\n' +
                    ...
                    console.log('Adding user ' + message.user.id + ' to overflow queue while previous match is created. You will be moved to the main queue shortly.');
                    overflowIds.push(message.user.id);
                    message.channel.send('Added ' + users[message.author.id].name + ' to overflow queue while previous match is created. You will be moved to the main queue shortly.');
                }
            } else {
                message.channel.send(users[message.author.id].name + ' is already in a match with ID `' + userActiveGame + '`');
            }
        } else {
            message.channel.send(users[message.author.id].name + ' is already in queue');
        }
    };
    ...
    obj.leave = function(message, matches) {
        ...
        if(queueId !== -1 && overflowId !== -1) {
            ...
            message.channel.send(users[message.author.id].name + ' removed from queue');
        } else {
            message.channel.send(users[message.author.id].name + ' cannot be removed from queue as they are not queued');
        }
